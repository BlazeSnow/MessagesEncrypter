...
`ver = 1` 仅定义 RSA 消息加密格式。X25519 等新算法留到 `ver = 2` 或后续版本再定义。
...
X25519 不进入 `ver = 1`。如果后续加入 X25519，建议在 `ver = 2` 中重新定义：

- 算法标识字段，例如 `alg`
- `alg = "x25519"`
- 每条消息携带发送方临时 X25519 公钥 `epk`
- 使用 X25519 shared secret + HKDF-SHA256 派生 AES-256-GCM 会话密钥
- 明确 HKDF `salt`、`info` 与 AES-GCM AAD 规则

## 修订记录

| 日期 | 版本 | 说明 |
| --- | --- | --- |
| 2026-05-31 | v1 draft | 记录 RSA Base64 JSON 结构，并将 X25519 延后到 v2 或后续版本。 |
