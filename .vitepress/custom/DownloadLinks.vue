...
const allLinks = computed(() =>
    [...presetLinks.value, ...(props.links ?? [])].map((link) => ({
        ...link,
        //站点部署在非根路径时，内部地址需要补上 base
        href: /^https?:\/\//i.test(link.href) ? link.href : withBase(link.href),
        image: link.image ? withBase(link.image) : undefined,
        imageDark: link.imageDark ? withBase(link.imageDark) : undefined
    }))
)
</script>
...
<template>
    <div class="download-links">
        <VPLink v-for="link in allLinks" :key="link.href" class="download-links__item" :href="link.href" target="_blank"
            rel="noopener noreferrer" no-icon>
            <img v-if="link.image && !link.imageDark" class="download-links__badge" :src="link.image"
                :alt="link.alt || link.label" loading="lazy" />
            <img v-else-if="link.image" class="download-links__badge download-links__badge--light" :src="link.image"
                :alt="link.alt || link.label" loading="lazy" />
            <img v-if="link.imageDark" class="download-links__badge download-links__badge--dark" :src="link.imageDark"
                :alt="link.alt || link.label" loading="lazy" />
            <span v-else class="download-links__button">
                <span class="download-links__button-label">{{ link.label }}</span>
                <svg class="download-links__button-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    aria-hidden="true">
