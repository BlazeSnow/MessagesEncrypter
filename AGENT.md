...
不要为了减小包体积重新开启裁剪或 ReadyToRun，除非已经验证 Release 打包启动和 WinUI 资源加载没有问题。
...
- 常规按钮可以使用 `FontIcon` 加文字；如图标含义不明显，应补充工具提示或明确按钮文字。

## 存储与设置

当前存储分工：

- 应用设置：使用 `ApplicationData.Current.LocalSettings.Values`，目前保存导出位置 `ExportFolderPath`。
- 密钥列表：保存到应用本地数据目录 `LocalState\keys.json`。
- 私钥密码：用户选择记住时保存到 Windows 凭据管理器，凭据类型为普通凭据。

不需要额外实现 `settings.json`。小型设置优先放入 `LocalSettings`，密钥和较复杂数据继续使用 `LocalState` 文件。
